...
function Engine3D(canvas, opts) {
  ...
  let dragging = false, panning = false, lastX = 0, lastY = 0, pinchDist = 0;
  const listeners = [];
  ...
  canvas.addEventListener('wheel', e => {
    ...
    cam.dist = Math.max(2, Math.min(80, cam.dist));
    changed();
  }, { passive: false });
  ...
  return { cam, project, begin, flush, line, arrow, sphere, poly, box, label, onChange };
}
